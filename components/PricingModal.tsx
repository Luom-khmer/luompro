...
const PricingModal: React.FC<PricingModalProps> = ({ isOpen, onClose, onSelectPackage }) => {
  ...
  return (
    <div className="fixed inset-0 z-[150] bg-black/90 flex items-center justify-center p-4 backdrop-blur-md overflow-hidden">
      <div className={`w-full max-w-7xl relative transition-all duration-500 ${selectedId ? 'opacity-0 pointer-events-none' : 'opacity-100'} h-full flex flex-col justify-center`}>
        
        {/* Close Button (Fades out when selecting) */}
        {!selectedId && (
            <button 
                onClick={onClose} 
                className="absolute top-4 right-4 text-white/70 hover:text-white transition-colors p-2 bg-white/10 rounded-full z-50 hover:rotate-90 duration-300"
            >
        ...
        {/* Promotion Banner */}
        {config && (
            <div className="bg-gradient-to-r from-yellow-50 to-orange-50 rounded-2xl p-6 mb-8 text-center shadow-lg border border-orange-200 shrink-0 mx-4 lg:mx-0">
        ...
        {/* We use a separate layout for the cards to allow "fixed" positioning animation */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 items-center justify-center px-4 lg:px-0">
            {/* 
               We map the config packages, but actually render them. 
               The animation trick: When selectedId is set, the selected card changes position style 
               to fixed centered, scaling up. Others fade out.
            */}
        </div>
      </div>

      {/* Render Cards Layer - Positioned absolutely to handle transitions better or sticking to grid?
          Actually, let's keep them in the grid but use CSS transforms on selection.
      */}
      
      <div className="absolute inset-0 overflow-y-auto overflow-x-hidden flex items-center justify-center p-4 md:p-8">
         <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 w-full max-w-7xl transition-all duration-500`}>
            {config?.packages.map((pkg) => {
                ...
                if (isSelected) {
                    // Zoom Effect: Fixed Center
                    containerClasses += ` fixed top-1/2 left-1/2 z-[200] shadow-[0_0_100px_rgba(255,255,255,0.8)] ring-8 ring-white/50`;
                    styleObj = {
                        ...styleObj,
